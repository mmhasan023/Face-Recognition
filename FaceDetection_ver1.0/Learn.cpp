...
void InitTrain()
{
	...
	if(dlg.DoModal()!=IDOK)
	{
		AfxMessageBox(L"Training cancelled.");
		return;
	}
	else
	{
	...
	if(i!=1 && i<=max_nodes)
	{
		...
		{
			AfxMessageBox(L"All bootstrapping file used! Training finished! (finally...)");
			return;
		}
...
const bool CascadeClassifier::OneRound(const int round)
{
	...
	str.Format(L"Training node: %d",round);
	...
	str.Format(L"Training node %d finished. Bootstrapping non-face data for next node.",round);
	pwnd->SetWindowText(str);
