...
//Dongseon Im 640*480으로 입력 이미지 고정
		IAMStreamConfig* m_pVStreamConfig=NULL;

		hr = g_pCapture-> FindInterface(&PIN_CATEGORY_CAPTURE,&MEDIATYPE_Interleaved, 
			pSrcFilter, IID_IAMStreamConfig, (void **)&m_pVStreamConfig);
		...
		if (hr != NOERROR)
		{
			...
			if (hr != NOERROR)
			{
				// this means we cant set frame rate (non-DV only)
				MessageBox(NULL, L"Error : Cannot find VCapture:IAMStreamConfig",NULL,MB_OK);
